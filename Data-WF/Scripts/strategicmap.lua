	...
	if ( bSectorZ == 0 ) then
		...
		if ( sSectorX == 13 and sSectorY == SectorY.MAP_ROW_D) then
			if ( GetTownLoyaltyRating(2) > 50 ) then
				-- wine store
				CreateCivilian(12369, 0, 55, Bodytype.REGFEMALE, -1, -1, -1, -1, -1, -1, -1, -1)
			end
			-- general store
			CreateCivilian(7313, 0, 41, Bodytype.MANCIV, -1, -1, -1, -1, -1, -1, -1, -1)
		-- Alma
		elseif ( sSectorX == 14 and sSectorY == SectorY.MAP_ROW_I) then
			-- wine store
			CreateCivilian(15428, 0, 56, Bodytype.MANCIV, Vest.GREYVEST, -1, -1, -1, -1, -1, -1, -1)
			-- restaurant
			CreateCivilian(13470, 0, 57, Bodytype.REGFEMALE, Vest.JEANVEST, Pants.GREENPANTS, -1, -1, -1, -1, -1, -1)
			-- general store
			CreateCivilian(13689, 0, 42, Bodytype.FATCIV, Vest.YELLOWVEST, Pants.BLUEPANTS, -1, -1, -1, -1, -1, -1)
		-- Cambria
		elseif ( sSectorX == 9 and sSectorY == SectorY.MAP_ROW_F) then
			-- drug dealer (university ruins)
			CreateCivilian(10510, 0, 60, Bodytype.HATKIDCIV, -1, -1, -1, -1, -1, -1, -1, -1)
		elseif ( sSectorX == 9 and sSectorY == SectorY.MAP_ROW_G) then
			-- pharmacy
			CreateCivilian(17181, 0, 47, Bodytype.FATCIV, Vest.WHITEVEST, -1, Hair.WHITEHEAD, -1, -1, -1, -1, -1)
			-- restaurant
			CreateCivilian(10468, 0, 49, Bodytype.MANCIV, Vest.GYELLOWSHIRT, Pants.BEIGEPANTS, -1, -1, -1, -1, -1, -1)
			-- general store
			CreateCivilian(16237, 0, 43, Bodytype.MANCIV, -1, -1, -1, -1, -1, -1, -1, -1)
		-- Grumm
		elseif ( sSectorX == 1 and sSectorY == SectorY.MAP_ROW_G) then
			-- beach bar
			CreateCivilian(20864, 0, 58, Bodytype.REGFEMALE, -1, -1, -1, -1, -1, -1, -1, -1)
		elseif ( sSectorX == 1 and sSectorY == SectorY.MAP_ROW_H) then
			-- general store
			CreateCivilian(10787, 0, 44, Bodytype.BIGMALE, -1, -1, -1, -1, -1, -1, -1, -1)
			-- restaurant
			CreateCivilian(12575, 0, 50, Bodytype.MINICIV, -1, -1, -1, -1, -1, -1, -1, -1)
			-- tool shop
			CreateCivilian(7737, 0, 61, Bodytype.REGFEMALE, -1, -1, -1, -1, -1, -1, -1, -1)
		-- Balime
		elseif ( sSectorX == 12 and sSectorY == SectorY.MAP_ROW_L) then
			-- luxury store
			CreateCivilian(17667, CivGroup.LOYAL_CIV_GROUP, 64, Bodytype.MANCIV, -1, -1, -1, -1, 212, 264, -1, -1)
		-- gas station near Balime
		elseif ( sSectorX == 10 and sSectorY == SectorY.MAP_ROW_L) then
			-- restaurant
			CreateCivilian(15120, 0, 51, Bodytype.MINICIV, -1, -1, -1, -1, -1, -1, -1, -1)
		-- San Mona
		...
		elseif ( sSectorX == 5 and sSectorY == SectorY.MAP_ROW_C) then
			...
			(CheckMercIsDead ( Profil.KINGPIN ) == false) ) then
				-- general store
				CreateCivilian(4563, CivGroup.KINGPIN_CIV_GROUP, 45, Bodytype.REGMALE, -1, -1, -1, -1, 694, 107, 37, -1)
				-- construction materials (also in general store)
				CreateCivilian(3927, CivGroup.KINGPIN_CIV_GROUP, 63, Bodytype.STOCKYMALE, -1, -1, -1, -1, 13, 161, 302, 135)
				-- armour store (back room of Frank's bar)
				CreateCivilian(13521, CivGroup.KINGPIN_CIV_GROUP, 67, Bodytype.MANCIV, -1, -1, -1, -1, 692, 72, 164, -1)
				-- brothel bar
				CreateCivilian(13069, CivGroup.KINGPIN_CIV_GROUP, 52, Bodytype.REGFEMALE, -1, -1, -1, -1, 759, -1, -1, -1)
			end
		-- Chitzena
		elseif ( sSectorX == 1 and sSectorY == SectorY.MAP_ROW_A) then
				-- bar
				CreateCivilian(11815, 0, 59, Bodytype.REGFEMALE, -1, -1, -1, -1, -1, -1, -1, -1)
		elseif ( sSectorX == 2 and sSectorY == SectorY.MAP_ROW_A) then
				-- pool bar
				CreateCivilian(18297, 0, 66, Bodytype.DRESSCIV, -1, -1, -1, -1, -1, -1, -1, -1)
		-- Meduna
		elseif ( sSectorX == 4 and sSectorY == SectorY.MAP_ROW_O) then
			-- posh general store
			CreateCivilian(8750, CivGroup.LOYAL_CIV_GROUP, 46, Bodytype.STOCKYMALE, -1, -1, -1, -1, 212, -1, -1, -1)
			CreateCivilian(9353, CivGroup.LOYAL_CIV_GROUP, 46, Bodytype.MANCIV, -1, -1, -1, -1, 212, -1, -1, -1)
			-- restaurant
			CreateCivilian(12269, CivGroup.LOYAL_CIV_GROUP, 53, Bodytype.MINICIV, -1, -1, -1, -1, -1, -1, -1, -1)
			-- restaurant
			CreateCivilian(12392, CivGroup.LOYAL_CIV_GROUP, 54, Bodytype.REGFEMALE, -1, -1, -1, -1, -1, -1, -1, -1)
			-- luxury store
			CreateCivilian(9366, CivGroup.LOYAL_CIV_GROUP, 65, Bodytype.MANCIV, -1, -1, -1, -1, 264, -1, -1, -1)
			-- pharmacy
			CreateCivilian(10437, CivGroup.LOYAL_CIV_GROUP, 48, Bodytype.FATCIV, -1, -1, -1, -1, -1, -1, -1, -1)
		end
	elseif ( bSectorZ == 1 ) then
		...
		if ( sSectorX == 10 and sSectorY == SectorY.MAP_ROW_A) then
			CreateCivilian(11435, CivGroup.REBEL_CIV_GROUP, 40, Bodytype.CRIPPLECIV, Vest.BROWNVEST, Pants.GREENPANTS, Hair.WHITEHEAD, -1, 711, 545, 545, -1)
		end
	end
